...
from var import Var
from Destroyer import CMD_LIST, LOAD_PLUG, SUDO_LIST, bot
from Destroyer.Configs import Config
from Destroyer.wraptools import (
    am_i_admin,
    ignore_bot,
    ignore_fwd,
    ignore_grp,
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        import Destroyer.modules
        import Destroyer.utils

        path = Path(f"Destroyer/modules/{shortname}.py")
        name = "Destroyer.modules.{}".format(shortname)
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = Destroyer.utils
        sys.modules["fire.util"] = Destroyer.utils
        sys.modules["userbot.utils"] = Destroyer.utils
        sys.modules["userbot.plugins"] = Destroyer.modules
        sys.modules["plugins"] = Destroyer.modules
        sys.modules["userbot"] = Destroyer
        mod.Config = Config
        ...
        sys.modules["Destroyer.events"] = Destroyer.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["Destroyer.modules." + shortname] = mod
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"Destroyer.modules.{shortname}"
...
def Destroyer_on_cmd(pattern=None, **args):
    args["func"] = lambda e: e.via_bot_id is None
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[Here](https://t.me/TEAM-DESTROYER-ON-STRICK)"
            ...
            ftext += "--------BEGIN DESTROYER USERBOT TRACEBACK LOG--------"
            ftext += "\nDate: " + date
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Destroyer/modules/assistant/{shortname}.py")
        name = "Destroyer.modules.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        spec.loader.exec_module(mod)
        sys.modules["Destroyer.modules.assistant" + shortname] = mod
