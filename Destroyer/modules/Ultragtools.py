...
from Destroyer.modules.sql_helper.mute_sql import is_muted, mute, unmute
from Destroyer.utils import Destroyer_on_cmd
...
async def gspider(Destroyer):
    ...
    a = b = 0
    if Destroyer.is_private:
        user = Destroyer.chat
        reason = Destroyer.pattern_match.group(1)
    else:
        Destroyer.chat.title
    try:
        user, reason = await get_full_user(Destroyer)
    except:
        pass
    try:
        if not reason:
    ...
    if user:
        ...
        try:
            from Destroyer.modules.sql_helper.gmute_sql import gmute
        except:
            pass
        try:
            await Destroyer.client(BlockRequest(user))
        except:
            pass
        testDestroyer = [
            d.entity.id
            for d in await Destroyer.client.get_dialogs()
            if (d.is_group or d.is_channel)
        ]
        for i in testDestroyer:
            try:
                await Destroyer.client.edit_permissions(
...
async def gspider(Destroyer):
    ...
    me = await Destroyer.client.get_me()
    await chris.edit(f"Trying To Ungban User !")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await Destroyer.get_chat()
    a = b = 0
    if Destroyer.is_private:
        user = Destroyer.chat
        reason = Destroyer.pattern_match.group(1)
    ...
    if user:
        ...
        try:
            from Destroyer.modules.sql_helper.gmute_sql import ungmute
        except:
            pass
        try:
            await Destroyer.client(UnblockRequest(user))
        except:
            pass
        testDestroyer = [
            d.entity.id
...
async def handler(rkG):
    if rkG.user_joined or rkG.user_added:
        try:
            from Destroyer.modules.sql_helper.gmute_sql import is_gmuted
